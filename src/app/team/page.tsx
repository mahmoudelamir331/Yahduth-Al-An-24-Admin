...
import { ChevronDown, Eye, EyeOff, ShieldCheck, UserPlus, Users } from "lucide-react";
...
export default function TeamPage() {
  ...
  async function addMember() {
    ...
    const response = await fetch("/api/team", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, name, password, role, permissions: perms }) });
    const result = (await response.json()) as { message?: string; error?: string };
    if (!response.ok) setError(result.error ?? "حدث خطأ");
    else { setMessage(result.message ?? "تم إنشاء الموظف"); setShowForm(false); setName(""); setEmail(""); setPassword(""); setShowPassword(false); setPerms({}); await load(); }
    setBusy(false);
  }
