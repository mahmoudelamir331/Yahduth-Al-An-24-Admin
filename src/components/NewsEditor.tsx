...
import { AlignCenter, AlignLeft, AlignRight, Bold, ImagePlus, Italic, Link, List, Quote, Redo2, Undo2, Video } from "lucide-react";
...
const YouTubeEmbed = Node.create({
  ...
  parseHTML: () => [{ tag: "iframe[data-youtube-embed]" }],
  renderHTML: ({ HTMLAttributes }) => ["iframe", { ...HTMLAttributes, "data-youtube-embed": "true", width: "100%", height: "315", frameborder: "0", allowfullscreen: "true", class: "aspect-video w-full rounded-lg border" }],
});
...
export default function NewsEditor({ onChange, initialContent = "" }: { onChange?: (html: string) => void; initialContent?: string }) {
  const editor = useEditor({
    extensions: [
      StarterKit,
      Placeholder.configure({ placeholder: "اكتب تفاصيل الخبر هنا..." }),
      TextStyle,
      Color,
  ...
  const addYoutube = () => { const url = window.prompt("اكتب رابط يوتيوب"); const embed = url ? toYoutubeEmbed(url) : ""; if (embed) editor.chain().focus().insertContent({ type: "youtubeEmbed", attrs: { src: embed } }).run(); };
  const addImage = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (response.ok && result.url) editor.chain().focus().setImage({ src: result.url, alt: file.name }).run();
  };

  return <div className="overflow-hidden rounded-lg border"><div className="tiptap-toolbar border-b bg-muted/30 p-2">
    ...
    <span className="toolbar-separator" aria-hidden="true" /><button type="button" onClick={() => run(() => editor.chain().focus().setTextAlign("right").run())} className="toolbar-button" aria-label="محاذاة يمين"><AlignRight size={16} /></button><button type="button" onClick={() => run(() => editor.chain().focus().setTextAlign("center").run())} className="toolbar-button" aria-label="محاذاة وسط"><AlignCenter size={16} /></button><button type="button" onClick={() => run(() => editor.chain().focus().setTextAlign("left").run())} className="toolbar-button" aria-label="محاذاة شمال"><AlignLeft size={16} /></button>
    <label className="toolbar-button cursor-pointer" aria-label="إضافة صورة"><ImagePlus size={16} /><input type="file" accept="image/*" className="hidden" onChange={addImage} /></label><button type="button" onClick={addYoutube} className="toolbar-button" aria-label="إضافة فيديو يوتيوب"><Video size={16} /></button><button type="button" onClick={addLink} className="toolbar-button" aria-label="إضافة رابط"><Link size={16} /></button><label className="flex h-8 items-center gap-1 rounded px-1 text-xs" title="لون الخط"><span className="sr-only">لون الخط</span><input type="color" onChange={(event) => editor.chain().focus().setColor(event.target.value).run()} /></label><button type="button" onClick={() => editor.chain().focus().undo().run()} className="toolbar-button" aria-label="تراجع"><Undo2 size={16} /></button><button type="button" onClick={() => editor.chain().focus().redo().run()} className="toolbar-button" aria-label="إعادة"><Redo2 size={16} /></button>
