...
export function Sidebar({ permissions }: { permissions: Record<AdminPermission, boolean> }) {
  ...
  return <>
    ...
    <aside className={`${open ? "translate-x-0" : "translate-x-full"} fixed right-0 top-0 z-20 flex h-screen w-72 flex-col border-l bg-card p-5 transition-transform lg:static lg:z-0 lg:w-64 lg:translate-x-0`}>
      ...
      <div className="mt-auto space-y-3"><div className="rounded-xl bg-accent p-3 sm:rounded-2xl sm:p-4"><BarChart3 className="mb-2 text-primary sm:mb-3" size={21} /><p className="text-sm font-bold">غرفة الأخبار</p><p className="mt-1 text-xs text-muted-foreground">تابع البيانات الحقيقية من لوحة الإحصائيات.</p></div><button type="button" onClick={logout} className="interactive-button flex w-full items-center gap-3 rounded-lg border px-3 py-2.5 text-xs font-semibold text-muted-foreground hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700 dark:hover:bg-rose-950/30 sm:text-sm" aria-label="تسجيل خروج"><LogOut size={17} />تسجيل خروج</button></div>
    </aside>
  </>;
}
